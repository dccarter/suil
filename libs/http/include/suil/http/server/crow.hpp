...
#ifndef SUIL_HTTP_SERVER_CROW_HPP
#define SUIL_HTTP_SERVER_CROW_HPP
...
namespace suil::http::crow {
    ...
    namespace magic {
        ...
        public:
            ...
            constexpr const_str(const char* arr, unsigned n)
                : begin_{arr}, size_{n}
            {}
...
#define INTERNAL_PARAMETER_TAG(t, i) \
        ...
        INTERNAL_PARAMETER_TAG(suil::String, 4);
    ...
    namespace detail {
        ...
        struct routing_params {
            struct strparams {
                ...
                strparams(suil::String str)
                    : str{std::move(str)}
                {}

                strparams(strparams&& other)
                    : str{std::move(other.str)}
                {}

                strparams &operator=(strparams&& other) {
                    str = std::move(other.str);
                    return *this;
                }

                operator suil::String() {
                    return str.peek();
                }
            };
            ...
            struct params_container {
                ...
                template <typename U = _T>
                    requires std::is_same_v<strparams, U>
                void push(U&& t) {
                    grow();
                    params[back++] = std::move(t);
                }
                ...
                _T& operator[](unsigned idx) const {
                    if (params && idx < back) {
                        return params[idx];
                    }
            ...
            private:
                inline void grow() {
                    if (params == nullptr) {
                        size = 4;
                        params = (_T *) calloc(1, sizeof(_T) * 8);
                    } else if (back == size) {
                        size *= 2;
                        params = (_T *) realloc(params, sizeof(_T) * size);
                        memset(&params[back], 0, (size-back)*sizeof(_T));
                    }
                }
            };
        ...
        template<>
        inline void routing_params::push<int64_t>(int64 v) {
            int_params.push(v);
        }

        template<>
        inline void routing_params::push<uint64_t>(uint64 v) {
            uint_params.push(v);
        }
        ...
        template<>
        inline void routing_params::push<suil::String>(suil::String v) {
            strparams str{std::move(v)};
            string_params.push(std::move(str));
        }
        ...
        template<>
        inline void routing_params::pop<suil::String>(suil::String) {
            string_params.pop();
        }
        ...
        template<>
        inline suil::String routing_params::get<suil::String>(unsigned index) const {
            auto &p = string_params[index];
            return p.str.peek();
        }
    }
}

#endif //SUIL_HTTP_SERVER_CROW_HPP
